...
struct Impl<MMA_81616, T_, KvQuant_, CTA_H_, CTA_Q_, CTA_S_, WARP_H_, WARP_Q, WARP_S, HeadDim, Stages> {
    ...
    static constexpr int XK = 16 / bitsof<TK>;
    static constexpr int XV = 16 / bitsof<TV>;
    ...
    using SmemLayoutK = decltype(_SmemLayoutKV(bitsof<TK>));
    using SmemLayoutV = decltype(_SmemLayoutKV(bitsof<TV>));

    using SmemLayoutKVp = SmemLayoutV2<CTA_S, 2, CTA_S, 2, Identity>;

    using PointerK = get_pointer_type<TK>;
    using PointerV = get_pointer_type<TV>;

    union SharedStorage {
        __align__(16) T Q[SmemLayoutQ::kSize];

        struct {
            __align__(16) Array<TK, Stages * SmemLayoutK::kSize> KV;
    ...
    using ThreadMapK = RakedThreadMap<HeadDim, CTA_S, 128 / bitsof<TK>, kWarpCount>;
    using ThreadMapV = RakedThreadMap<HeadDim, CTA_S, 128 / bitsof<TV>, kWarpCount>;
        ...
        __device__ void Load(int k, int pipe_iter)
        {
            ...
            if (k % XK == 0) {
                ...
                for (int m = 0; m < K_M; ++m) {
                    ...
                    static_assert(sizeof(data_K[k / XK][m]) == 16);
        ...
        __device__ void Load(int m, int pipe_iter)
        {
            ...
            if (m % XV == 0) {
                ...
                for (int k = 0; k < V_K; ++k) {
                    ...
                    static_assert(sizeof(data_V[m / XV][k]) == 16);
                    if constexpr (!kQuantKV) {
                        ldsm_x4_trans(
