...
template<class T, class KvQuant, int Qh>
using KT = AttentionUniversal<
    arch::Sm75,
    Mainloop<arch::Sm70, Impl<MMA_81616, T, KvQuant, Qh, 1, kCTA_S, Qh, 1, kWARP_S, kHeadDim, kStages>>,
    GetBlockIterFactory<T, KvQuant, kCTA_S, kHeadDim>,
    DecodingCtaMap>;

namespace {
Registrar reg([](Collector& c) {
    c.add<KT<half, KvQuantNone, 8>>();
    c.add<KT<half, KvQuantNone, 16>>();

    c.add<KT<half, KvQuantInt8, 8>>();
    c.add<KT<half, KvQuantInt8, 16>>();

    c.add<KT<half, KvQuantInt4, 8>>();
    c.add<KT<half, KvQuantInt4, 16>>();
});
}
