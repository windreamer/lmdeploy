...
int test_turboquant_decode()
{
    ...
    std::cout << "head_dim=" << kHeadDim << " kv_heads=" << KvHeadNum << " batch=" << kBatchSize
              << " seq_len=" << kSequenceLen << " quant_policy=" << kQuantPolicy << "\n";
    ...
    std::cout << "K roundtrip:\n";
    Compare(k_out, k_cache_ref.data().get(), kContextLen * kHeadDim, kContextLen * kHeadDim, kBatchSize * KvHeadNum, 0);
    std::cout << "V roundtrip:\n";
    Compare(v_out, v_cache_ref.data().get(), kContextLen * kHeadDim, kContextLen * kHeadDim, kBatchSize * KvHeadNum);
    ...
    params.block_iter_params = BlockIteratorParams{k_ptrs.data().get(), cu_block_cnts.data().get(), 0, (int)kBlockSz};
    ...
    {
        std::vector<T> h_out(output.size());
        cudaMemcpy(h_out.data(), output.data().get(), h_out.size() * sizeof(T), cudaMemcpyDeviceToHost);
        ...
        std::cout << "Output: " << h_out.size() << " values, " << nan_count << " NaN, " << inf_count << " Inf, "
                  << zero_count << " zero\n";
        // Print first 16 non-zero, non-NaN values
        std::cout << "First values: ";
        int printed = 0;
        for (size_t i = 0; i < h_out.size() && printed < 16; ++i) {
            float v = __half2float(h_out[i]);
            if (!std::isnan(v) && !std::isinf(v) && v != 0.f) {
                std::cout << v << " ";
                ++printed;
            }
        }
